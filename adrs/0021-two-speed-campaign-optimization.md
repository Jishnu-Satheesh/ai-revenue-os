...
- **A pause is proven by reading the object back, never by the provider acknowledging the write.** Acknowledgement says the request was accepted; only the object's own delivery status says it stopped. A pause is idempotent, so unlike a publish it can be re-sent safely, and the verifying read is attempted even when the write timed out — a timed-out pause that actually landed settles there instead of escalating.
- **Containment may reach above the ad.** Pausing the ad is what stops delivery and is what the allocation loop does. A guardrail or ceiling stop, and any case where a build's own outcome was unknown and the ad's provider id was never learned, escalates to the ad set and campaign: an ad created under a paused campaign inherits the stop whether this platform can name it or not. Escalation still pauses; it never deletes.
