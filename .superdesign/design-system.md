...
As of 2026-09-11 the user-approved purpose is **the home of the organization**. The organization identity, actual campaigns and creative assets lead; goals, attention, workspace destinations and recent recorded activity support them. The prior analytics-led cockpit/report designs are historical, not instructions for the next implementation.

Use `docs/superpowers/specs/2026-09-11-organization-home-design.md` and its implementation/data/visual companions. The current application still renders `OverviewReport` until that handoff is executed. Do not infer implementation approval or completed browser acceptance from a design artifact.

Preserve the existing Manrope/neutral/emerald platform language and AppShell. Real organization artwork supplies visual identity; do not generate sample records, choose ambiguous logos, retheme the shell, or create an AI briefing duplicate of Growth Intelligence. Campaigns use saved states and current-version covers; finished poster renders remain distinct from raw campaign images and reviewed brand references. Home is read-first: navigation and read-only previews, with existing role-gated organization management retained below.
