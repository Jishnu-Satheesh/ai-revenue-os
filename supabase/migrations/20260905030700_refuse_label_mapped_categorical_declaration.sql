-- Corrects a claim `20260903130000_declare_projection_categorical_value.sql`
-- made and got wrong for one shape of output.
--
-- That migration's header says appending one value to an already-legal
-- `allowedValues` array "produces a document that is legal by the exact same
-- rules an operator's own hand-typed declaration would have to satisfy," and
-- concludes no guard needs touching. That is true only where the output has
-- no `labelMap`. Where one exists,
-- `private.assert_report_projection_document` (see
-- `20260902174000_admit_projection_categorical_value_separator.sql:153-184`)
-- requires the map to reach *every* allowed value -- "a code nothing maps to
-- can never be written" -- so appending a bare code with no map entry to
-- reach it produces a document `assert_report_projection_matches_contract`
-- refuses with `22023 report projection categorical label map is invalid`.
-- That refusal is real and correct; the prior migration's comment asserting
-- the append always succeeds is not, for this one shape. This migration does
-- not edit that file -- it is applied and live on staging -- it corrects the
-- record here and fixes the gap.
--
-- A label map exists precisely for providers whose raw column text is prose
-- ("Cancelled by merchant"), which is exactly the shape most likely to turn
-- up as an undeclared value in the first place. So this is not a corner
-- case: it is the routine one for any output built with a map.
--
-- Two ways to close this were considered. Teaching the declare path to also
-- synthesize and insert a `labelMap` entry (mapping the provider's raw text
-- to a code) would make the one-click path work here too, but it would mean
-- this endpoint choosing a permanent vocabulary code on an operator's behalf
-- with no review -- exactly the judgement `report projection categorical
-- value is invalid`'s existing message already reserves for a hand-authored
-- declaration ("longer provider prose needs a label map in a hand-authored
-- declaration"). Refusing up front, with its own distinguishable error and
-- its own plain-English message, keeps that judgement where the rest of this
-- feature already puts it, and is the smaller change against a function the
-- reviewer already found sound. If that judgement changes later, it belongs
-- in a new migration and a new message case, not a silent broadening of this
-- one-click path.

create or replace function public.propose_governed_report_projection_with_declared_value(
...
begin
  ...
  -- A label map exists to translate this output's raw provider text into the
  -- approved vocabulary, and the document guard requires it to reach every
  -- allowed value (20260902174000, "a code nothing maps to can never be
  -- written"). Appending a bare code here, with no map entry naming it,
  -- would leave exactly that: refused later, at
  -- assert_report_projection_matches_contract, with a generic 22023 that
  -- names no cause. Refusing here first, distinctly, lets the screen say why
  -- instead of "refresh the evidence and try again."
  if (output_value -> 'categorical') ? 'labelMap' then
    raise exception 'report projection output uses a label map' using errcode = '23514';
  end if;
