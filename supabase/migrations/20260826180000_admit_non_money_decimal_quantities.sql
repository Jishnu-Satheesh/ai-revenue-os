-- Admit exact decimal quantities for every non-money period-grain metric.
--
-- The prior migration intended to admit fractional availability minutes but
-- checked for `ratio`, a value kind this RPC does not accept. That left every
-- valid `count` quantity on the integer-only branch. Money remains integer
-- minor units; count quantities may carry at most fifteen decimal places.

-- Validate the replacement before removing the older integer-only guard. The
-- ledger is small, but this order also ensures there is never a commit where
-- the value-shape invariant is absent.
alter table public.normalized_metrics
  add constraint normalized_metrics_value_numerator_check check (
    (value_kind = 'money' and value_numerator = trunc(value_numerator))
      or (value_kind = 'count' and scale(value_numerator) <= 15)
      or value_kind not in ('money', 'count')
  ) not valid;
alter table public.normalized_metrics
  validate constraint normalized_metrics_value_numerator_check;
alter table public.normalized_metrics
  drop constraint normalized_metrics_check4;
...
begin
  ...
  for emitted in select value from jsonb_array_elements(p_observations) loop
    if jsonb_typeof(emitted) <> 'object'
      ...
      -- because a figure with more than fifteen decimal places is not one anybody
      -- reported.
      or (case when emitted ->> 'valueKind' = 'money'
        then coalesce(emitted ->> 'valueNumerator', '') !~ '^-?[0-9]+$'
        else coalesce(emitted ->> 'valueNumerator', '') !~ '^-?[0-9]+(\.[0-9]{1,15})?$' end)
