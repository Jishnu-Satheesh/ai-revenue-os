-- Market monitoring Task 9 repair: derive a missing item branchId from the
-- claimed request instead of refusing it.
--
-- The base slice refused payloads without a branchId key on branch-scoped
-- requests. Every production caller sends the key (the repository boundary
-- rejects key-less items before any RPC), but the Task 8 pipeline suite
-- finalizes with key-less fixtures and must keep passing unmodified: its
-- items carry no citations, so derivation changes nothing it proves. An
-- explicit branchId that disagrees with the request still refuses, and the
-- claim/finding support checks below still run against the request branch
-- unconditionally — derivation only fills what the request already proves.
