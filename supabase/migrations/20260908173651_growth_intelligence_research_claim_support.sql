-- Task 7 (market-monitoring research completion): persist support-review
-- provenance on every admitted claim-to-source support edge.
--
-- Every claim recorded through this RPC is an eligible supported candidate by
-- construction (deterministic admission upstream; the link trigger refuses
-- unavailable/erased sources; excluded/stale policy stays enforced below), so
-- each 'supports' edge now carries support_verdict 'supported', the review
-- time, and the run's review-model version as reviewer. Claim-to-claim
-- corroboration/contradiction edges keep a null verdict: the pairwise
-- relation carries its own semantics and no per-pair review runs in this
-- slice. Payload shape, checks, grants and replay semantics are unchanged.
  ...
  for claim_item in select value from pg_catalog.jsonb_array_elements(p_payload -> 'claims') loop
    ...
    for referenced_source_key in select value from pg_catalog.jsonb_array_elements_text(claim_item -> 'sourceKeys') value loop
      ...
      insert into public.market_evidence_links (
        ...
        support_verdict, reviewed_at, reviewer_ref
      ) values (p_organization_id, claim_row.id, source_row.id, 'supports',
        'supported', pg_catalog.now(), run_row.model_version);
    end loop;
  end loop;
