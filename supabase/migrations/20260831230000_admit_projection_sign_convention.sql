-- Admit a projection output that records a provider's deduction as a cost.
--
-- `financialSign` on a contract field asserts what the file contains; it does
-- not change it. That is right for validation and wrong for a cost. Keeta
-- writes commission as a positive in its order export and as a negative in its
-- billing report, and one metric cannot hold both and still be summable.
--
-- The bill this unblocks is real. Reconciling a client's Keeta statement of
-- account against what this platform records leaves a quarter of what the
-- marketplace invoiced unaccounted for: bank charges and POS machine fees, both
-- written as deductions in the billing report and both unrecordable until now
-- without landing under a cost metric with the sign reversed.
--
-- A named convention rather than a sign flip, for the same reason `convert` is
-- a named conversion rather than a multiplier: an operator approving "a
-- deduction, recorded as a cost" is approving something they can read. The
-- application layer additionally refuses it on any field the contract does not
-- already declare negative, so it can restate a deduction and can never quietly
-- invert a revenue column into a cost.
--
-- Replaced whole rather than patched: plpgsql has no way to amend a literal
-- allow-list in place. The body below is the live function from 20260830162749
-- with the allow-list widened by one key and one block added. Nothing else
-- differs.
