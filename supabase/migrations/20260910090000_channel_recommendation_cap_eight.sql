-- Amendment C (ADR 0053): the per-run narration cap rises from 6 to 8.
--
-- Six detector chapters plus two spare slots for needs_data notes, so the
-- prompt-v8 coverage rule (every chapter holding observation findings gets at
-- least one citing item) never competes with an honest data gap. Identical
-- body to 20260824110000 apart from the cap line; grants persist across
-- create or replace, so none are repeated here.
--
