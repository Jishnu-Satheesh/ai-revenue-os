...
-- import, and the operator needs to be told which problem they have.
--
--   TOTALS_ROW_NOT_RESOLVED   the contract says this export renders its own
--                             total, and the file has none, or has several
--
-- Neither outcome may be guessed past. Summing a totals row in with the rows it
-- totals doubles every figure while looking like a clean import, and picking
-- one of several matches is a guess dressed as a rule.
--
...
begin
  if p_failure_code not in ('OBJECT_IDENTITY_CHANGED', 'OBJECT_UNAVAILABLE', 'PACKAGE_EXPIRED',
    ...
    'TOTALS_ROW_NOT_RESOLVED', 'PROJECTION_PROCESSING_FAILED') or p_result_digest !~ '^[a-f0-9]{64}$' then
