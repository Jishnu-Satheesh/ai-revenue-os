-- Repair: `pg_catalog.greatest(...)` does not exist. GREATEST is a syntactic
-- construct the parser handles specially, not a catalog function, so it cannot
-- be schema-qualified even under `search_path = ''`. Unqualified is correct and
-- is not a search_path risk, because the parser resolves it before any schema
-- lookup happens.
