-- Admit a projection output that declares what a provider's own words mean.
--
...
-- chart compares across providers. Talabat already writes keys -- a closed day
-- says `CHECK_IN_REQUIRED`. Keeta writes English: `Cancelled by merchant`.
--
-- Until now the second kind was unreadable, so the one column in Keeta's order
-- export that says whose fault a cancellation was could not be projected at
-- all, and the workspace counted cancellations with no fault attached.
--
-- A declared map rather than inference. Upper-casing, stripping spaces or
-- matching loosely would put the same judgement in code, where nobody approves
-- it and nobody sees it change. Approved in the document, an operator reads
-- "Cancelled by merchant means MERCHANT" and can say whether that is true.
--
-- The refusal ADR 0034 rests on is unchanged: text the map does not carry
-- still stops the import rather than becoming an "other" nobody defined.
--
-- Replaced whole rather than patched: plpgsql has no way to amend a literal
-- allow-list in place. The body below is the live function from 20260831230000
-- with the categorical allow-list widened by one key and one block added.
  ...
  for output in select value from jsonb_array_elements(p_document -> 'outputs') loop
    ...
    if output ? 'categorical' then
      ...
      if jsonb_typeof(categorical) <> 'object'
        or (select bool_or(key not in ('dimensionKey', 'allowedValues', 'collectInjectedValues', 'labelMap')) from jsonb_object_keys(categorical) key)
