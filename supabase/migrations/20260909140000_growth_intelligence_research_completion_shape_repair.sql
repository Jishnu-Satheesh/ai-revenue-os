-- Task 8 repair 5: null fields must fail the synthesis shape gates.
--
-- SQL three-valued logic let a missing resultDigest, items array, or
-- failure code slip past the shape checks (NULL comparisons yield NULL,
-- never true) and fail later with a less precise error. The gates now
-- treat missing values as invalid up front, the way the shared research
-- result validator already does with coalesce. No other behavior changes.
