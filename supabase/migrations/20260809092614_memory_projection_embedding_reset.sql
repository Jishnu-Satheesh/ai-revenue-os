-- Forward replacement: a changed provider projection invalidates only its
-- derived vector state while retaining Task 11's verification reset semantics.
  ...
  do update set
    ...
    embedding_model = case
      when memory_items.branch_id is not distinct from excluded.branch_id
        ...
        and memory_items.observed_at is not distinct from excluded.observed_at
        and memory_items.source_reference is not distinct from excluded.source_reference
      then memory_items.embedding_model else null end,
    embedding_status = case
      when memory_items.branch_id is not distinct from excluded.branch_id
        ...
        and memory_items.observed_at is not distinct from excluded.observed_at
        and memory_items.source_reference is not distinct from excluded.source_reference
      then memory_items.embedding_status else 'pending' end,
    embedding_updated_at = case
      when memory_items.branch_id is not distinct from excluded.branch_id
        ...
        and memory_items.observed_at is not distinct from excluded.observed_at
        and memory_items.source_reference is not distinct from excluded.source_reference
      then memory_items.embedding_updated_at else null end,
