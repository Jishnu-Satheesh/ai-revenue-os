  ...
  for emitted in select value from jsonb_array_elements(p_observations) loop
    if jsonb_typeof(emitted) <> 'object'
      ...
      or (case when emitted ->> 'valueKind' = 'ratio'
        then coalesce(emitted ->> 'valueNumerator', '') !~ '^-?[0-9]+(\.[0-9]{1,15})?$'
...
-- Follow-up in the same slice: providers measure availability in fractional
-- minutes, which ADR 0036 admits as continuous quantities. The gate below now
-- accepts exact decimals (fifteen places or fewer) for every non-money kind.
-- Money stays integer-strict: it travels in minor units by convention, and a
-- fractional amount would mean the mapping itself is wrong.
