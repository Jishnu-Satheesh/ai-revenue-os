-- Amendment C (ADR 0053): one gap-fill narration per already-narrated run.
--
-- A run whose narration left findings uncited admits exactly one second
-- submission restricted to previously-uncited findings, inside the same
-- per-run cap of 8. The claim answers `gapfill_acquired` while an uncited
-- finding exists (and fewer than two tellings are stored); the completion
-- path refuses fully-cited resubmissions as before, mixed cited/uncited
-- submissions outright, and run totals above 8. Bodies are otherwise
-- identical to 20260824110000; grants persist across create or replace.
--
