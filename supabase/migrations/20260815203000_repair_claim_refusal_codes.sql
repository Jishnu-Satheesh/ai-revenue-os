-- Repair: every `codes := codes || 'literal'` in `claim_campaign_action` failed
-- with "malformed array literal". Concatenating a `text[]` with an untyped
-- literal makes Postgres read the literal as an array, so the first refusal the
-- function tried to record raised instead. Each append now casts to `text`.
--
-- Caught by the pgTAP suite on the first refusal path, which is the point of
-- executing a new plpgsql function rather than trusting that it applied.
