-- Task 8 repair: replay must precede the lease gate in synthesis finalization.
--
-- The base migration asserted the synthesis lease before checking whether
-- the run already committed. A duplicate child execution after commit (its
-- request already succeeded or failed) therefore raised claim_lost instead
-- of replaying the committed outcome. Both synthesis RPCs now lock
-- request, pipeline and run first, answer committed duplicates identically,
-- and only then enforce the lease gate for genuinely new work. No other
-- behavior changes.
