-- Growth Intelligence Task 16 plan repair: the approved plan requires an
-- Opportunity read to return its stored action key, but no session-readable
-- stored key existed (playbook_versions grants no select to authenticated and
-- carries no read policy). The worker already stamps the selected playbook's
-- key into the aggregate JSON, so opportunities now persist it beside every
-- other denormalized decision field and session reads return the stored value
-- instead of a code default. Existing rows backfill from their playbook
-- version; the worker insert requires the key in the strict contract.
alter table public.opportunities add column action_key text;

update public.opportunities as opportunity
set action_key = playbook.action_definition ->> 'action_key'
from public.playbook_versions as playbook
where playbook.organization_id = opportunity.organization_id
  and playbook.id = opportunity.playbook_version_id;

alter table public.opportunities alter column action_key set not null;

alter table public.opportunities add constraint opportunities_action_key_format
  check (action_key ~ '^[a-z][a-z0-9_.-]{1,120}$');
