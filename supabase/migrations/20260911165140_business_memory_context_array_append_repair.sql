-- Task C breaker repair: text[] || unknown-literal resolves to array-concat
-- (22P02). array_append is the correct append. Mechanical transform of the live
-- prepare_memory_context_core; see ledger breaker note. Next task reviewer owns
-- this file in its surface.

CREATE OR REPLACE FUNCTION private.prepare_memory_context_core(p_organization_id uuid, p_purpose text, p_consumer_kind text, p_consumer_id uuid, p_attempt_key text, p_correlation_id uuid, p_branch_id uuid, p_channel_id uuid, p_campaign_id uuid, p_policy_version text, p_entries jsonb, p_retrieval_latency_ms integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
  ...
  begin
    ...
    for v_index in 0 .. v_length - 1 loop
      ...
      if v_sensitivity not in ('public', 'internal') then
        v_excl_codes := array_append(v_excl_codes, 'SENSITIVITY_BLOCKED');
        continue;
      end if;
      ...
      if v_element ->> 'scopeBranchId' is null then
        ...
        if v_count = 0 then
          v_excl_codes := array_append(v_excl_codes, 'SCOPE_BLOCKED');
          continue;
        end if;
      else
        raise exception 'memory context entry is invalid' using errcode = '23514';
      ...
      loop
        ...
        if not v_drop.opt then
          ...
          if not ('MANDATORY_OVERFLOW' = any(v_reasons)) then
            v_reasons := array_append(v_reasons, 'MANDATORY_OVERFLOW');
          end if;
          v_excl_codes := array_append(v_excl_codes, 'MANDATORY_OVERFLOW');
        else
          v_excl_codes := array_append(v_excl_codes, 'OVER_BUDGET');
        end if;
      end loop;
    end if;
