-- Forward repair for 20260911144805 / 20260911160031 (Spec 023 Task C, fix round 4).
--
-- COALESCE is syntax, not a function: pg_catalog.coalesce(...) does not exist
-- (42883), so every prepare/revalidate/erase path using it aborts at first
-- execution. The four affected private/public functions are recreated here
-- with bodies identical except pg_catalog.coalesce( -> coalesce(. Valid
-- qualifications (pg_catalog.chr/now/encode) are kept exactly as they are.
-- Refinement recorded: qualify real functions only; never qualify COALESCE,
-- NULLIF, or CAST constructs. Grants untouched.
...
begin
  ...
  if p_source_kind = 'memory_item' then
    ...
    select coalesce(pg_catalog.array_agg(erased_event.id), '{}') into v_event_ids
    ...
    select coalesce(pg_catalog.array_agg(erased_item.id), '{}') into v_item_ids
