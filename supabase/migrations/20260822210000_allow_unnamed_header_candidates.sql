-- Names may cover fewer rows than digests, and every named row must be one of
-- them.
--
...
-- are one-way — but its text is the client's actual numbers, and the profiler
-- now refuses to name any row where a cell reads as a figure. So a sheet can
-- legitimately offer three digest candidates and one named one.
--
-- Requiring the two arrays to be equal length, as the version this replaces
-- did, would reject exactly that. The rule becomes the one that was meant: a
-- named row must correspond to a candidate row that was actually profiled, and
-- there can be no more names than candidates.
