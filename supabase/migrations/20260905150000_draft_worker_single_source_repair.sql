-- Growth Intelligence Task 22 repair: the worker set both brief and opportunity
-- sources, violating the campaigns exactly-one-source rule; assigned
-- `campaign_id = campaign_id` (a no-op self-assignment); and omitted the goal
-- metric the snapshot freeze requires. Drafts reference their opportunity;
-- objective and audience freeze into the snapshot facts.
