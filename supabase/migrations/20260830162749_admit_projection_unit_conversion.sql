-- Admit a projection output that converts the provider's unit to the registry's.
--
-- Keeta reports open and closed time in hours; the availability detector reads
-- `operations.closed_minutes` and `operations.scheduled_minutes`. Storing hours
-- under a key that says minutes is a lie with a units label on it.
--
-- A named conversion rather than a multiplier. A free `scale: 60` says nothing
-- about why, cannot be reviewed, and is one step from the arbitrary expression
-- this declaration language deliberately does not have. The list is closed, so
-- an unknown name refuses the document instead of passing the value through.
--
-- Replaced whole rather than patched: plpgsql has no way to amend a literal
-- allow-list in place. The body below is the live function from 20260829133515
...
begin
  ...
  for output in select value from jsonb_array_elements(p_document -> 'outputs') loop
    if jsonb_typeof(output) <> 'object'
      or coalesce((select bool_or(key not in ('key', 'normalizedSheetName', 'canonicalField', 'metricKey', 'valueKind', 'aggregation', 'categorical', 'sumWith', 'convert')) from jsonb_object_keys(output) key), false)
