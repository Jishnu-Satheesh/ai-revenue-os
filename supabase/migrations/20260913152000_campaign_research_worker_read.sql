-- Task 6 worker reads: admission manifest binding plus claim-bound loading (C03).
--
-- The requester pins the subject pack with their own session before a run is
-- admitted; the worker must never re-derive it. So admission accepts the
-- pinned manifest identity (identifiers only), and the worker loads the
-- pinned entries through a claim-bound loader — the same shape as
-- `load_campaign_generation_context`. Sensitive text travels in the pinned
-- manifest behind its own access rules, never in a Trigger payload.

-- 1. Admission accepts the pinned manifest --------------------------------------
--
-- Forward-compatible replace: the two new keys are optional, and every call
-- the approval suite already proved still behaves identically without them.
...
begin
  ...
  -- A pinned manifest travels as identifiers only, both or neither. The
  -- manifest must exist for this tenant: a dangling pin admits nothing.
...
-- 2. Claim-bound worker load ------------------------------------------------------
--
-- Everything the worker needs in one call: the run, the still-binding policy
-- version, and the pinned manifest with its exact bounded entry snapshots.
-- The claim token and a live lease are required, so a worker that lost its
-- lease loads nothing. Erased entries arrive as ids with empty snapshots;
-- the planner treats content-less entries as absent, never as evidence.

create function public.load_campaign_research_context(
  target_organization_id uuid,
  input_load jsonb
)
returns jsonb
...
revoke all on function public.load_campaign_research_context(uuid, jsonb)
  from public, anon, authenticated;
grant execute on function public.load_campaign_research_context(uuid, jsonb) to service_role;
