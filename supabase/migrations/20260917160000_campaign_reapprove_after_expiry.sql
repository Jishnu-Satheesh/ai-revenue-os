...
--
-- approve_campaign_bundle never retired expired rows, so the one-live-
-- per-version index blocked "approve again" forever while the UI invited
-- it. The function now retires expired-unrevoked rows for the version
-- before inserting; a still-live row raises campaign_approval_already_live.
-- Whole body repeated for CREATE OR REPLACE; grants untouched.
