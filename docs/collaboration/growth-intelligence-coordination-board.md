...
- 2026-09-22 Ready-review card round 2 (this session): same two files. Featured card -> `01-market-watch-desktop.png` rhythm: full-bleed footer divider, edge-to-edge tinted rail (sm:w-60, pl-5), h-10 buttons (Review px-5 + arrow), badge radius-md, title 22px with mt-3/1.5/2.5 rhythm. Agent toggle is now shadcn Switch (primary-green active) in the footer button group after Project history; row toggles share the component. No API/RDBMS/trigger changes.
- 2026-09-22 Report-reader prototype parity (approved detailed plan, this session): touching ONLY `src/components/growth-intelligence/report-reader.tsx` (+ its `.test.tsx`). Green eyebrows, 24px bold dialog title, 208px rail with green active item, 32px article padding, short [S1] citation labels, gaps info icon, footer 28/16px padding with h-10 buttons + arrow. Removes "What it means" block from view (data untouched); keeps finding-checkbox disabled state + note (backend Tier 3 outstanding); keeps real footer copy/labels. No API/RDBMS/trigger changes.
- 2026-09-22 Report-reader round 3 (approved detailed plan, this session): same two files. Fixed dialog frame sm:h-[85vh] (90vh cap, mobile dvh kept), competitors 22px bold heading, 16/20px table cells, 28% name column, dark populated rows vs muted zero-citation rows, estimate padding 24px. WHY IT MATTERS column stays out (no data field; Tier 3 follow-up). No API/RDBMS/trigger changes.
- 2026-09-22 Report-reader round 4 (approved detailed plan, this session): same two files. Sources tab: 22px bold heading, digest line removed, records as short [S1] labels plus guarded domain titles, URL + retrieval lines kept, 20px row rhythm. No kind/description lines (fields do not exist; not invented). No API/RDBMS/trigger changes.
- 2026-09-22 Report-reader round 5 (approved detailed plan, this session): same two files. Opportunity tab: 22px bold heading, static 3-item pre-commit checklist (prototype words, fixed copy like the honesty box), honesty box with info icon + bold title + roomier padding. No intro cites (no data). No API/RDBMS/trigger changes.
- 2026-09-22 Workspace literal-parity (approved literal plan, this session): rewriting `src/components/growth-intelligence/insights-list.tsx` (rows + Improve-rail), DELETING `data-gaps.tsx`, editing workspace wiring + `workspace-sections.test.tsx` + `growth-intelligence-workspace.test.tsx`. Insight/gap feedback controls lose their UI (routes+data stay). No API/RDBMS/trigger/assembler changes.
- 2026-09-22 History-dialog prototype parity (approved detailed plan, this session): touching `src/components/growth-intelligence/project-overview-dialog.tsx` (+ test) and section wiring in `market-watch-projects.tsx` (brief-number map). Prototype header/rows/footer; takeaway out of rows; pause/stop/history preserved; View-update disabled with reason. No API/RDBMS/trigger changes.
- 2026-09-22 History-dialog round 2 (approved detailed plan, this session): same files. Drop REPORT HISTORY caps, unconditional update row (bare-state subline without revision), footer two-group gap guard. Verify brief map shapes (code-level; shapes align). No API/RDBMS/trigger changes.
- 2026-09-22 Workspace batch (five user items, this session): remove insights intro Card; New-research button to h-10 px-5; Insights tab 2nd; history dialog footer Close-only; rail 3-item cap + shadcn Drawer (new `ui/drawer.tsx` + vaul dep) for full gap list. Files: workspace, insights-list, project-overview-dialog (+tests), new drawer. No API/RDBMS/trigger changes.
