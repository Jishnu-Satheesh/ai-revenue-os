...
- [x] Link a saved approval to real Campaign preparation in Your actions; preserve route/tab context, error/retry, latest update and the actual ready creative-review link.
...
- [x] Reconcile the dormant `createGrowthIntelligenceOpportunitySource` with actual SQL/worker admission. Reuse its useful semantics, but do not remove legacy execution gates or generate fictitious numeric impact to populate a list.
...
- Status 2026-09-16 (second pass): decided proposals now reach Your actions — the timeline carries proposal rows (snoozes and dismissals reuse the ordinary types so the existing filters catch them; approving and asking for changes get their own, because "planned" is not an approval), and an approved proposal shows what became of it with a link to the campaign it opened. The dormant source is reconciled as **stays dormant, by design**, with the reasoning written at the top of the file: `qualifyDraftImpact` demands a governed impact range, declared confidence, the author's own words for it, stated assumptions and source revisions, and `SynthesizedItemRow` carries no impact fields at all — wiring it would mean inventing those numbers.
- **Blocked, verified against staging 2026-09-16.** The remaining boxes wait on two breaks upstream of every surface here, both recorded in `docs/collaboration/campaign-and-growth-intelligence-seam.md`. (1) Nothing calls `researchCampaignProposalTask.trigger(...)` and no route calls `request_campaign_research_run`, so no research ever starts. (2) The research worker runs as `service_role`, and `20260913120000` revokes `request_campaign_proposal` and `complete_campaign_proposal_version` from `service_role` — probed live, it gets `42501 permission denied`. Both bodies also require `auth.uid()`, so a grant alone is not enough. Closing (2) is Tier 3 (it moves a control/execution boundary) and needs an approved Execution Plan. Making the manual request proposal-first is deliberately held until then: an entry point that produced only failing runs would be worse than none.
