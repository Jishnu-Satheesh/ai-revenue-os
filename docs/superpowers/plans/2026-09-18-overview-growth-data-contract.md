...
- Read the trailing reported window: the 30 local days ending at the source cutoff (named constants GROWTH_BASELINE_WINDOW_DAYS, GROWTH_BASELINE_MIN_REPORTED_DAYS = 7). The baseline monthly level is the observed daily mean over REPORTED days only, scaled to a 30-day standard month (GROWTH_BASELINE_STANDARD_MONTH_DAYS). Missing days are excluded and disclosed, never zero-filled or invented. A day counts as reported only when every frozen partition covers it exactly once at the finest granularity available: an exact day cover wins over a coarser containing span (which is then ignored for that day); a single containing span contributes its amount spread evenly over its own reported days; two disagreeing covers of one day refuse the candidate (BASELINE_INCOMPLETE), never pick a winner. At least 7 reported days are required, and the latest reported day must end no more than 45 local days before issue (GROWTH_BASELINE_MAX_AGE_DAYS). Uniform currency and conflict-free coverage stay hard refusals. This replaces the 2026-09-18 complete-prior-calendar-month rule (ADR 0066): a new organization publishes within days of reporting, and one missing day no longer voids a month.
- Fallback ladder (ADR 0066 amendment, 2026-09-21): when the 30-day window holds fewer than 7 reported days, widen rung by rung and take the first rung that qualifies — 14 reported days in the trailing 60 days, 21 in 90, 28 in 120 (GROWTH_BASELINE_FALLBACK_RUNGS). Older data earns its place with more of it. The 45-day freshness bound on the latest reported day, uniform currency and conflict-free coverage apply to every rung; a rung that cannot trust its days refuses outright instead of widening past the defect. An empty window refuses at every rung. The limitation names the rung actually used (count, span, latest day).
- This spread is baseline-mean arithmetic only: it divides a reported interval total over its own reported days to form the monthly level. It never creates daily actual observations — D05 display still draws measured blue only where exact cover exists, and the frozen document stores no per-day baseline breakdown.
- A count of rows or the latest arbitrary analysis bucket does not prove coverage. Use the per-day cover assessment above, not D05's cumulative endpoint algorithm (which stays the display-actuals rule).
