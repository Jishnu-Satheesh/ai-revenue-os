...
- [x] **Step 1: Write RED budget and adapter tests.** Covered at the adapter: currency mismatch, ceiling above the reservation, no reservation at all, zero ceiling, partial provider object creation, idempotent resume, unknown outcome mid-build, and both local and provider stop. **Provider minimum and maximum budget are deliberately not enforced.** Meta's per-currency, per-billing-event daily minimums are not proven by any source in the contract, and inventing a floor would be exactly the invented platform limit AGENTS.md forbids; they remain a `blocked` restriction until controlled-account evidence exists. Approved audience and placement, creative-to-version match, account spend cap, and scope loss belong to the request loader that resolves a `CappedExperimentRequest`, which is Task 20's dispatch work — the adapter never widens what it is handed.
- [x] **Step 2: Implement `meta.ads.run_bounded_experiment`.** Create or resume the verified provider object graph from one approved action. Persist every provider external ID before the next call, so a retry cannot duplicate a campaign, ad set, creative, or ad object. Map variants to ads one to one.
- [x] **Step 3: Enforce spend twice.** The claim's reservation is now handed down to the adapter, which is what made the second enforcement real rather than nominal: the adapter refuses a ceiling above what the database committed, refuses a currency that differs from it, and refuses a money-moving action with no reservation — all before any provider object exists. The provider-side half is the ad set's `daily_budget` and hard `end_time`, which hold if nothing here runs again. The receipt carries approved, reserved and settled figures in one currency so the comparison needs no inference.
- [x] **Step 4: Register `meta.ads.pause_ad` as its own tool.** Its own tool key, action run, idempotency key and unknown-outcome path. Two things fell out of building it that are now in ADR 0021. A pause is proven by reading the object's `effective_status` back, never by the provider acknowledging the write — and because a pause is idempotent, that read is attempted even after a timed-out write, so a pause that actually landed settles instead of escalating. And containment escalates above the ad for a guardrail or ceiling stop, or whenever the build's outcome was unknown and the ad's id was never learned, since an ad nobody can name can only be stopped from above it. An empty ledger is a refusal, not an all-clear.
...
- [x] **Step 6: Verify and commit.** Idempotent resume, direct RPC misuse, cross-tenant attachment, immutability and object-kind shape covered by 18 pgTAP assertions against staging; both new plpgsql functions executed there, satisfying the standing rule that a function is not done until it has been called once for real. Typecheck clean, lint 0 errors, full suite green apart from two pre-existing UI suites this work never touched.
