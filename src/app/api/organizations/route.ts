...
import { listOrganizations } from "@/domain/organizations/repository";
...
export async function POST(request: Request) {
  try {
    ...
    await createEventPublisher().publish({
      ...
      schemaVersion: 1,
      payload: { slug: organization.slug, industryPackSlug: organization.industry_pack_slug },
    });
...
export async function GET() {
  try {
    ...
    if (!user) throw new DomainError("AUTHENTICATION_ERROR", "Authentication is required.");
    return NextResponse.json({ organizations: await listOrganizations(supabase) });
  } catch (error) {
    const publicError = toPublicError(error);
